...
public class PrometeoCarController : MonoBehaviour
{
    ...
    */
    ...
    void OnEnable()
    {
        carControls.Car.Enable();
        
        carControls.Car.Throttle.performed  += ctx => isThrottlePressed  = true;
        carControls.Car.Throttle.canceled   += ctx => isThrottlePressed  = false;

        carControls.Car.Reverse.performed   += ctx => isReversePressed   = true;
        carControls.Car.Reverse.canceled    += ctx => isReversePressed   = false;

        carControls.Car.TurnLeft.performed  += ctx => isTurnLeftPressed  = true;
        carControls.Car.TurnLeft.canceled   += ctx => isTurnLeftPressed  = false;

        carControls.Car.TurnRight.performed += ctx => isTurnRightPressed = true;
        carControls.Car.TurnRight.canceled  += ctx => isTurnRightPressed = false;

        carControls.Car.Handbrake.performed += ctx => isHandbrakePressed = true;
        carControls.Car.Handbrake.canceled  += ctx => {
    ...
    void Update()
    {
        ...
        if (!isThrottlePressed && !isReversePressed)
        {
            ThrottleOff();
        }
            ...
            try
            {
                if (carEngineSound != null)
                {
                    float engineSoundPitch =
                        initialCarEngineSoundPitch + (Mathf.Abs(carRigidbody.linearVelocity.magnitude) / 25f);
